...
function sendMail($email, $token)
{
    ...
    try {
        ...
        $mail->Body    = "We got a request from you to change the password.<br>
                            ...
                            <a href='http://localhost/PHP/third_year_project/enews/update-password.php?email=$email&token=$token'>Reset Password</a>";
...
if (isset($_POST['submit'])) {
    $query = "SELECT * FROM `user` WHERE 'emailId'='$_POST[email]'";
    $result = mysqli_query($conn, $query);
    if ($result) {
        $token = bin2hex(random_bytes(16));
        $date = date("Y-m-d");
        $query = "UPDATE `user` SET `token`='$token',`forgotExpiry`='$date' WHERE `emailId`='$_POST[email]'";
