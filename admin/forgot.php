...
function sendMail($email, $token)
{
    ...
    try {
        ...
        $mail->Body    = "We got a request from you to change the password.<br>
                            ...
                            <a href='http://localhost/PHP/third_year_project/enews/admin/update-password.php?email=$email&token=$token'>Reset Password</a>";
...
if (isset($_POST['submit'])) {
    $query = "SELECT * FROM `admin` WHERE 'emailId'='$_POST[email]'";
    $result = mysqli_query($conn, $query);
    if ($result) {
        $token = bin2hex(random_bytes(16));
        $date = date("Y-m-d");
        $query = "UPDATE `admin` SET `token`='$token',`forgotExpiry`='$date' WHERE `adminEmail`='$_POST[email]'";
