...
<body>
    ...
    if (isset($_POST['updatePassword'])) {
        ...
        if(mysqli_num_rows($result2)){
            if ($_POST['password'] == $_POST['confirmPassword']) {
                ...
                $update = "UPDATE `user` SET `password`='$password',`token`=NULL,`forgotExpiry`=NULL WHERE `username`='$_POST[username]'";
    ...
    if (isset($_GET['email']) && isset($_GET['token'])) {
        $date = date("Y-m-d");
        $query = "SELECT * FROM `user` WHERE `emailId`='$_GET[email]' AND `token` = '$_GET[token]' AND `forgotExpiry`='$date'";
